...
We're trying to work with capabilities, so what capability do we need to solve this? Well, we've seen that our solution relies on marking a place in the code to jump back to if necessary, so that's what we'll need: a marker of the position to jump to.
...
The first part is specific to what we're doing, so we can't really abstract over it. But this notion of what to do on success and failure? _That_ we can allow callers to customize.

For that, we'll need to make a more general version of `boundary`, one with more type parameters:
- `E` (for error), the type of values passed to `break`.
- `A`, the type of the value returned if the computation reaches its natural end.
- `O` (for output), what's actually returned by the `boundary` block.

We'll then need some way of turning `E` and `A` into `O`: this is how we allow success and failures to be of different types, but then unify them to a single output type.
...
def handle[E, A, O](
  ...
  catch case Break(`label`, value) => failure(value)
```
...
```scala
def option[E, A](oa: Label[E] ?=> A): Option[A] =
  handle(oa, Some.apply, _ => None)
```
...
```scala
def either[E, A](ea: Label[E] ?=> A): Either[E, A] = 
  handle(ea, Right.apply, Left.apply)
```
...
This is problematic, because `Iterator` is lazy. The `_.?` bit won't be executed until _after_ `sequenceIterator` exits - outside of prompt's `try` / `catch` block, allowing the `Break` exception to go unchecked and result in a runtime failure.
...
This is a typical problem of value escaping, which Scala now has a mechanism to deal with: [capture checking](./capture_checking.html). We can make the problem disappear by flagging the `Label` as tracked:
...
Of course, the compiler is then quick to remind us that we need to update our `option` prompt to track its label as well:
...
With this relatively simple change, `broken` fails to compile with a slightly sibylline error message: we can no longer jump to a "dead" label.

This might seem like a relatively minor point, but is quite important: capabilities are scoped to regions of a program, and must only be available within that scope. Allowing them to escape leads to all sort of painful, hard to debug problems. Capabilities really need capture checking in order to be safe and remain easy to reason about.
