...
We'll start by solving the second problem only: generating a list of `n` random elements for a given `n`. It will look something like this:
...
And so, if we wanted a list of lower case characters of arbitrary length, we could simply write:

```scala
val letters: Rand ?-> List[Char] = 
...
Now - `letters` is not _quite_ right yet, is it? It generates random lists of lower case letters. We first want to have it return a string, which is exactly as simple as you'd hope:
```scala
val letters: Rand ?-> String = 
  listOfN(int(100), lowerAscii)
    .mkString
```
...
The second improvement we want to bring to `letters` is, we need it to also contain upper case letters. This will require a little more legwork.

### Choosing between two generators
In order to allow `letters` to generate either upper or lower case letters, we need the ability to merge two generators (`upperAscii` and `lowerAscii`) into a single one.
...
We're almost done. We have almost all the parts we need to write our `identifier` generator. The one missing bit is: `or` isn't quite powerful enough yet. A valid variable name can contain letters _and_ digits, and so I'd like a convenient way of merging more than two generators.
...
The _body_ of the following function is more or less trivial. It's type though? Yeah. Rough edges.
...
Having written all that, we can finally write what I set out to do in the first place: a generator for _identifiers_, that is, non-empty strings whose first character is a letter, but later characters can also be digits.
